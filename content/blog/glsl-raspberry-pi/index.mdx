...
A solution to prevent GPU frequency down-scaling, is to set the minimum frequency, by adding `v3d_freq_min=500` to the `/boot/config.txt` file[^12].

[^12]: This is not optimal, as it increases power consumption, while the GPU is idle.
