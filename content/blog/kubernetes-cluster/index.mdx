...
excerpt: >
  ...
  As a software developer, I either use a local single-node Kubernetes cluster, or a remote multi-node cluster, to test my work on a daily basis.
...
| Part                                                           | Source                                  |
| -------------------------------------------------------------- | --------------------------------------- |
| Pico 5S Raspberry Pi starter kit case                          | [picocluster.com][picocluster]          |
| 2x Raspberry Pi 4B (4GB), 3x Raspberry Pi 3B+                  |                                         |
...
| Heschen 12V 25A SPST 2-Pin ON/OFF switch                       | [heschen.com][heschen]                  |
| Noctua NF-A4x20 5V PWM fan                                     | [noctua.at][noctua]                     |
| MOSFET breakout board module switch relay (15A, 30V) 4-outputs | [ebay.com][mosfet]                      |
| 2x JMS58 based M.2 NVMe to USB 3.0 adapters                    | [amazon.com][amazon-shinestar]          |
| 2x Samsung SSD 970 EVO Plus M.2 PCIe NVMe 500 Go               |                                         |
...
| Breadboard Dupont female to female jumper wires ribbon cables  |                                         |

<!-- | WR-WTB 4 pins - 2.54mm left angled header bottom locking | [we-online.com][we-online] |
| 1.5 kΩ 0.5W resistor | |
| 2P shunt 5.08 mm | | -->

[picocluster]: https://www.picocluster.com/collections/raspberry-pi/products/pico-5-raspberry-pi
...
[dehner]: http://store.dehner.net/27049/en
...
[heschen]: https://www.heschen.com/product-page/heschen-metall-toggle-switch-flick-flip-12-v-25-a-spst-on-off-2-angekommen-2-pin
...
That explains I still have three RPi3s as worker nodes. I replaced the two others with RPi4s, for nodes requiring more resources, _i.e._, the master node, and the worker node also used for data backup.
...
The measurements are compatible with the estimatation I did.
...
I had naively assumed any adapters would work, so I bought a first adapter, without checking it's compatibility with the RPi4.
...
I got disappointed with the result, as 39MB/s is far from saturating the rate the gigabit switch can achieve.
I started investigating what could possibly be the bottleneck, and realized one CPU core was always at 100% each time I transfered a file.
...
A quick test using `netcat` gave a much better transfer rate of 104MB/s:
...
I also plan to use SSD drives for USB mass storage booting, as soon as it's fully supported by the RPi4.
...
I would recommand the awesome [k8-on-raspbian](https://github.com/teamserverless/k8s-on-raspbian) tutorial by _Alex Ellis_.
...
Retrieve the token, that's necessary for the worker nodes to connect to the master node:
...
I plan to write a series of follow-up blog posts, that would specifically cover some areas in more depth, _e.g._:
...
- Setting up [USB mass storage booting](https://www.raspberrypi.org/documentation/hardware/raspberrypi/bootmodes/msd.md) with SSD drives;
